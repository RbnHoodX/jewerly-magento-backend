// ROBUST Data Validation Script for Production Import
...
class RobustDataValidator {
  ...
  async validateAllData(): Promise<void> {
    console.log("🔍 Starting ROBUST data validation for production import...");
    console.log("=" * 60);
    ...
    try {
      ...
      if (isSafe) {
        ...
        console.log("🔄 Rollback available if needed: npm run rollback rollback");
      } else {
  ...
  private async validateMainOrders(): Promise<void> {
    ...
    for (let i = 0; i < ordersData.length; i++) {
      ...
      const validation = this.validateOrderDataRobust(orderData, i + 1);
  ...
  private async validateCustomerNotes(): Promise<void> {
    ...
    for (let i = 0; i < notesData.length; i++) {
      ...
      const validation = this.validateCustomerNoteDataRobust(noteData, i + 1);
  ...
  private async validateDiamonds(): Promise<void> {
    ...
    for (let i = 0; i < diamondsData.length; i++) {
      ...
      const validation = this.validateDiamondDataRobust(diamondData, i + 1);
  ...
  private async validateCasting(): Promise<void> {
    ...
    for (let i = 0; i < castingData.length; i++) {
      ...
      const validation = this.validateCastingDataRobust(casting, i + 1);
  ...
  private async validateThreeD(): Promise<void> {
    ...
    for (let i = 0; i < threeDData.length; i++) {
      ...
      const validation = this.validateThreeDDataRobust(threeD, i + 1);
  ...
  private async validateEmployeeComments(): Promise<void> {
    ...
    for (let i = 0; i < commentsData.length; i++) {
      ...
      const validation = this.validateEmployeeCommentDataRobust(comment, i + 1);
  ...
  // ROBUST validation methods that handle all real-world data issues
  private validateOrderDataRobust(orderData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    try {
      ...
      const billingFirstName = this.safeString(orderData["Billing First Name:"]);
      const billingLastName = this.safeString(orderData["Billing Last Name"]);
      ...
      const email = this.safeString(orderData["Customer Email"]);
      if (email && !this.isValidEmail(email)) {
        errors.push(`Invalid email format: ${email}`);
      }

      // Phone validation (handle null/undefined)
      const phone = this.safeString(orderData["Billing Tel"]);
      if (phone && phone.trim() !== "") {
      ...
      const orderDate = this.safeString(orderData["Order Date"]);
      if (orderDate && orderDate.trim() !== "") {
        if (!this.isValidDate(orderDate)) {
          errors.push(`Invalid date format: ${orderDate}`);
        }
      }

      // Amount validation (handle empty amounts)
      for (let i = 1; i <= 10; i++) {
        const price = this.safeString(orderData[`Price ${i}`]);
        const qty = this.safeString(orderData[`Qty ${i}`]);
        const subtotal = this.safeString(orderData[`Row Subtotal ${i}`]);

        if (price && price.trim() !== "" && !this.isValidAmount(price)) {
          errors.push(`Invalid price for product ${i}: ${price}`);
        }
        if (qty && qty.trim() !== "" && !this.isValidAmount(qty)) {
          errors.push(`Invalid quantity for product ${i}: ${qty}`);
        }
        if (subtotal && subtotal.trim() !== "" && !this.isValidAmount(subtotal)) {
  ...
  private validateCustomerNoteDataRobust(noteData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!noteData["Order #"]) errors.push("Missing Order #");
    
    // Comment is optional in real-world data
    const comment = this.safeString(noteData["Comment"]);
    if (!comment || comment.trim() === "") {
      // This is actually OK for customer notes
    }
    
    const dateAdded = this.safeString(noteData["Date Added"]);
    if (dateAdded && dateAdded.trim() !== "") {
      if (!this.isValidDate(dateAdded)) {
        errors.push(`Invalid date format: ${dateAdded}`);
      }
    }
  ...
  private validateDiamondDataRobust(diamondData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!diamondData["Order #"]) errors.push("Missing Order #");
    
    // Type and Product are optional in real-world data
    const type = this.safeString(diamondData["Type"]);
    const product = this.safeString(diamondData["Product"]);
    ...
    const ctWeight = this.safeString(diamondData["CT Weight"]);
    if (ctWeight && ctWeight.trim() !== "" && !this.isValidAmount(ctWeight)) {
      errors.push(`Invalid CT Weight: ${ctWeight}`);
    }

    const totalPrice = this.safeString(diamondData["Total Price"]);
    if (totalPrice && totalPrice.trim() !== "" && !this.isValidAmount(totalPrice)) {
      errors.push(`Invalid Total Price: ${totalPrice}`);
    }
  ...
  private validateCastingDataRobust(castingData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!castingData["Order #"]) errors.push("Missing Order #");
    
    // Supplier is optional in real-world data
    const supplier = this.safeString(castingData["Supplier"]);
    if (!supplier || supplier.trim() === "") {
      // This is OK for casting
    }
    
    const metalType = this.safeString(castingData["Metal Type"]);
    if (!metalType || metalType.trim() === "") {
      errors.push("Missing Metal Type");
    }

    const qty = this.safeString(castingData["Qty"]);
    if (qty && qty.trim() !== "" && !this.isValidAmount(qty)) {
      errors.push(`Invalid quantity: ${qty}`);
    }

    const weight = this.safeString(castingData["Weight"]);
    if (weight && weight.trim() !== "" && !this.isValidAmount(weight)) {
      errors.push(`Invalid weight: ${weight}`);
    }

    const price = this.safeString(castingData["Price"]);
    if (price && price.trim() !== "" && !this.isValidAmount(price)) {
  ...
  private validateThreeDDataRobust(threeDData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!threeDData["Order #"]) errors.push("Missing Order #");
    
    const date = this.safeString(threeDData["Date"]);
    if (date && date.trim() !== "") {
  ...
  private validateEmployeeCommentDataRobust(commentData: any, row: number): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!commentData["Order #"]) errors.push("Missing Order #");
    
    // Comment is optional in real-world data
    const comment = this.safeString(commentData["Comment"]);
    if (!comment || comment.trim() === "") {
      // This is OK for employee comments
    }
    
    const dateAdded = this.safeString(commentData["Date Added"]);
    if (dateAdded && dateAdded.trim() !== "") {
      if (!this.isValidDate(dateAdded)) {
        errors.push(`Invalid date format: ${dateAdded}`);
      }
  ...
  private displayValidationResults(): void {
    console.log("\n" + "=" * 60);
    console.log("📊 ROBUST VALIDATION RESULTS");
    console.log("=" * 60);
  ...
  private isImportSafe(): boolean {
    ...
    // Import is safe if error rate is less than 15% (more lenient for real-world data)
    return errorRate < 15;
  }
}
...
async function main() {
  try {
    const validator = new RobustDataValidator();
