// Data Cleaning Script for Import Data
// Fixes common data quality issues before import
...
class DataCleaner {
  ...
  async cleanAllData(): Promise<void> {
    ...
    try {
      ...
      console.log("🚀 You can now run: npm run validate-data-robust");
  ...
  private async cleanMainOrders(): Promise<void> {
    ...
    const cleanedData = ordersData.filter(order => {
      ...
      if (!order["Billing First Name:"] || order["Billing First Name:"].trim() === "") return false;
      if (!order["Billing Last Name"] || order["Billing Last Name"].trim() === "") return false;
  ...
  private cleanDate(date: string): string {
    ...
    if (date.includes("0000-00-00")) return "";
