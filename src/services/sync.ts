...
export class SyncService {
  ...
  async syncOrders(orders: ShopifyOrder[]): Promise<SyncResult> {
        ...
        try {
          ...
          if (order.line_items && order.line_items.length > 0) {
            ...
            for (const lineItem of order.line_items) {
              try {
                const imageUrl = await this.shopify.getProductImageUrl(
                  lineItem
                );
                if (imageUrl) {
          ...
          // Check if order already exists by Shopify ID or order number
          const existingOrderIdByShopifyId =
            await this.db.getOrderIdByShopifyId(shopifyId);
          const existingOrderIdByOrderNumber =
            await this.db.getOrderIdByShopifyOrderNumber(order.name);
          const existingOrderId =
            existingOrderIdByShopifyId || existingOrderIdByOrderNumber;

          let orderId: string;

          if (existingOrderId) {
            // Log which method found the existing order
            const foundBy = existingOrderIdByShopifyId
              ? "Shopify ID"
              : "Order Number";
            this.logger.log("info", "Found existing order, updating", {
              shopifyId,
              name: order.name,
            ...
            const existingOrder = await this.db.getExistingOrderData(
              existingOrderId
            );

            // Delete all order-related data
            await pRetry(() => this.db.deleteOrderData(existingOrderId), {
            ...
            this.logger.log("debug", "Merged order data", {
              orderId: existingOrderId,
              shopifyId,
              name: order.name,
              mergedFields: Object.keys(mergedOrderData),
              totalAmount: mergedOrderData.total_amount,
            });
            ...
            await pRetry(
              async () => {
                try {
                  await this.db.recreateOrderData(
                    existingOrderId,
                    customerData,
                    mergedOrderData,
                    items,
                  ...
                  this.logger.log("error", "Failed to recreate order data", {
                    orderId: existingOrderId,
                    shopifyId,
                    name: order.name,
                    error:
            ...
            await this.db.updateOrderTrackingNote(
              existingOrderId,
              shopifyId,
              order.name
            );
          ...
          const errorMessage =
            err?.message || err?.details || err?.hint || String(err);
    ...
    await this.logger.writeJson(`${runId}-sync-summary.json`, syncResult);
