// Final test with a completely different approach
...
async function finalTest() {
  try {
    ...
    console.log("🔍 Final test with different approach...\n");
    ...
    const rows = parseCSVManually(csvText);
...
function parseCSVManually(csvText: string): string[][] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    for (const status of knownStatuses) {
      if (line.startsWith(status + ",")) {
        ...
        const row = parseDataRow(lines, i, knownStatuses);
        if (row) {
          rows.push(row);
        }
        break;
