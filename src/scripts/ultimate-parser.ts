...
async function ultimateParser() {
  try {
    ...
    const rows = parseCSVUltimate(csvText);
...
function parseCSVUltimate(csvText: string): string[][] {
  ...
  while (i < lines.length) {
    ...
    for (const status of knownStatuses) {
      if (line.startsWith(status + ",")) {
        ...
        const { rowData, nextIndex } = parseDataRowUltimate(
          lines,
          i,
          knownStatuses
        );
