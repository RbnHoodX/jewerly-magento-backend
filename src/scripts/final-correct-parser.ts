...
async function finalCorrectParser() {
  try {
    ...
    const rows = parseCSVFinalCorrect(csvText);
...
function parseCSVFinalCorrect(csvText: string): string[][] {
  ...
  while (i < lines.length) {
    ...
    for (const status of knownStatuses) {
      if (line.startsWith(status + ",")) {
        ...
        const { rowData, nextIndex } = parseDataRowFinalCorrect(
          lines,
          i,
          knownStatuses
        );
...
function parseDataRowFinalCorrect(
  allLines: string[],
  startIndex: number,
